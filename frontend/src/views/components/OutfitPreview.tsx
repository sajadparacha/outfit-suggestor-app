  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden transition-all hover:shadow-xl">
      ...
      <div className="p-8">
        ...
        {suggestion.matching_wardrobe_items && (
          (suggestion.matching_wardrobe_items.shirt?.length > 0 ||
           suggestion.matching_wardrobe_items.trouser?.length > 0 ||
           suggestion.matching_wardrobe_items.blazer?.length > 0 ||
           suggestion.matching_wardrobe_items.shoes?.length > 0 ||
           suggestion.matching_wardrobe_items.belt?.length > 0) && (
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-lg p-6 mb-6">
              <div className="flex items-start space-x-3 mb-4">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-800 mb-1">You Have These Items!</h3>
                  <p className="text-sm text-gray-600 mb-4">
                    These items from your wardrobe match the outfit suggestion. You can use them instead of buying new items!
                  </p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {suggestion.matching_wardrobe_items.shirt?.length > 0 && (
                      <div className="bg-white rounded-lg p-4 border border-green-200">
                        <div className="text-sm font-semibold text-green-700 mb-2">👕 Shirt ({suggestion.matching_wardrobe_items.shirt.length})</div>
                        <div className="space-y-2">
                          {suggestion.matching_wardrobe_items.shirt.map((item, idx) => (
                            <div 
                              key={item.id || idx} 
                              onClick={() => onNavigateToWardrobe && onNavigateToWardrobe('shirt')}
                              className="flex items-start gap-2 text-sm cursor-pointer hover:bg-green-50 p-2 rounded transition-colors"
                              title="Click to view in wardrobe"
                            >
                              {item.image_data && (
                                <img 
                              ...
                              <div className="flex-1">
                                <div className="font-medium text-gray-800">{item.color || 'Item'}</div>
                                {item.description && (
                    ...
                    {suggestion.matching_wardrobe_items.trouser?.length > 0 && (
                      <div className="bg-white rounded-lg p-4 border border-green-200">
                        <div className="text-sm font-semibold text-green-700 mb-2">👖 Trouser ({suggestion.matching_wardrobe_items.trouser.length})</div>
                        <div className="space-y-2">
                          {suggestion.matching_wardrobe_items.trouser.map((item: MatchingWardrobeItem, idx: number) => (
                            <div 
                              key={item.id || idx} 
                              onClick={() => onNavigateToWardrobe && onNavigateToWardrobe('trouser')}
                              className="flex items-start gap-2 text-sm cursor-pointer hover:bg-green-50 p-2 rounded transition-colors"
                              title="Click to view in wardrobe"
                            >
                              {item.image_data && (
                    ...
                    {suggestion.matching_wardrobe_items.blazer?.length > 0 && (
                      <div className="bg-white rounded-lg p-4 border border-green-200">
                        <div className="text-sm font-semibold text-green-700 mb-2">🧥 Blazer ({suggestion.matching_wardrobe_items.blazer.length})</div>
                        <div className="space-y-2">
                          {suggestion.matching_wardrobe_items.blazer.map((item: MatchingWardrobeItem, idx: number) => (
                            <div 
                              key={item.id || idx} 
                              onClick={() => onNavigateToWardrobe && onNavigateToWardrobe('blazer')}
                              className="flex items-start gap-2 text-sm cursor-pointer hover:bg-green-50 p-2 rounded transition-colors"
                              title="Click to view in wardrobe"
                            >
                              {item.image_data && (
                    ...
                    {suggestion.matching_wardrobe_items.shoes?.length > 0 && (
                      <div className="bg-white rounded-lg p-4 border border-green-200">
                        <div className="text-sm font-semibold text-green-700 mb-2">👞 Shoes ({suggestion.matching_wardrobe_items.shoes.length})</div>
                        <div className="space-y-2">
                          {suggestion.matching_wardrobe_items.shoes.map((item: MatchingWardrobeItem, idx: number) => (
                            <div 
                              key={item.id || idx} 
                              onClick={() => onNavigateToWardrobe && onNavigateToWardrobe('shoes')}
                              className="flex items-start gap-2 text-sm cursor-pointer hover:bg-green-50 p-2 rounded transition-colors"
                              title="Click to view in wardrobe"
                            >
                              {item.image_data && (
                    ...
                    {suggestion.matching_wardrobe_items.belt?.length > 0 && (
                      <div className="bg-white rounded-lg p-4 border border-green-200">
                        <div className="text-sm font-semibold text-green-700 mb-2">🪢 Belt ({suggestion.matching_wardrobe_items.belt.length})</div>
                        <div className="space-y-2">
                          {suggestion.matching_wardrobe_items.belt.map((item: MatchingWardrobeItem, idx: number) => (
                            <div 
                              key={item.id || idx} 
                              onClick={() => onNavigateToWardrobe && onNavigateToWardrobe('belt')}
                              className="flex items-start gap-2 text-sm cursor-pointer hover:bg-green-50 p-2 rounded transition-colors"
                              title="Click to view in wardrobe"
                            >
                              {item.image_data && (
